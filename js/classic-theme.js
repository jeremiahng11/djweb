...
Doodle.loadThemeAssets = function (game) {
  try {
    ...
    game.load.spritesheet("ufofly_space", "static/images/Playerfull/space/ufo-doodler.png?v=193", 211, 158, 9); // doodler-in-UFO ride (space, 480x360 -> 160x120 x9). ?v busts stale image cache
    game.load.image("ufopick_space", "static/images/Playerfull/space/ufo-power.png?v=193"); // empty saucer pickup (space). ?v busts stale image cache
    Doodle.THEMES.forEach(function (th) {
      ...
      if (d && d.jet) {
        game.load.image("jetpick_" + th, "static/images/PowerTiles/" + th + "_jetpack.png?v=198");                 // twin-rocket pickup (platform only)
...
// --- UFO ride (space): grab the saucer -> the doodler-in-UFO flies up like the rocket, then pops out. ---
// Art "ufofly_space" is a 3x3 grid (9 frames, 448x336) of the doodler in a UFO with an animated thruster.
Doodle.maybeUfo = function (gs, platform) {
  try {
    if (!platform || platform.hasBonusObject !== -1) return; // deterministic gate (same across clients)
    ...
    b.anchor.setTo(0.5, 0.82); b.scale.setTo(0.526, 0.526);  // ~158px saucer (matches the flying UFO size), disc bottom on the platform
...
Doodle.activateUfo = function (a, b, gs) {
  try {
...
// UFO flight: same ascent profile as the rocket, then the doodler pops back out (no empty vehicle to drop).
Doodle._ufoFlight = function (a, gs) {
  try {
    var pt = a.playerTimer, S = Phaser.Timer.SECOND / 60;
    pt.add(72 * S, function () { if (a.body) a.body.gravity.y = -1728; try { a.play("uf_fast"); } catch (e) {} });  // steady climb -> middle frames (medium flame)
    pt.add(285 * S, function () { if (a.body) a.body.gravity.y = 850; try { a.play("uf_slow"); } catch (e) {} });   // slowing down -> top frames (no flame)
    pt.add(330 * S, function () {
      try {
        ...
        if (a.body) {
          a.body.setSize(60, 90, a.width / 2 - 30, a.height / 2 - 45 + 20); // realign collision to the doodler
        ...
        if (Doodle._imgOK(g, pk)) {
          ...
          eu.anchor.setTo(0.5, 0.5); eu.scale.setTo(0.526, 0.526);
  ...
  } catch (e) { Doodle._show("ufoFlight: " + e.message); }
};
